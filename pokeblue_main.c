...
#include "assets_manifest_pokeblue.h"
extern uint8_t pokeblue__rom_data[];
extern const size_t pokeblue__rom_size;
static const GBGameAssets GB_POKEBLUE_GAME = {
    .game_id = "pokeblue",
    .rom_filename = "roms/pokeblue.gb",
    .rom_data = pokeblue__rom_data,
    .rom_size = 1048576u,
    .expected_sha1 = { 0xd7, 0x03, 0x7c, 0x83, 0xe1, 0xae, 0x5b, 0x39, 0xbd, 0xe3, 0xc3, 0x07, 0x87, 0x63, 0x7b, 0xa1, 0xd4, 0xc4, 0x8c, 0xe2 },
    .manifest = POKEBLUE_ASSETS_MANIFEST,
    .manifest_count = POKEBLUE_ASSETS_MANIFEST_COUNT,
};
...
int pokeblue_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *pokeblue_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "pokeblue");
        gb_context_set_save_id(interpreted_ctx, "pokeblue");
        pokeblue_init(generated_ctx);
        pokeblue_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "pokeblue");
    ...
    if (!gb_load_assets(&GB_POKEBLUE_GAME)) {
        fprintf(stderr, "[pokeblue] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_POKEBLUE_GAME.game_id);
#endif
    pokeblue_init(ctx);
    int exit_code = 0;
